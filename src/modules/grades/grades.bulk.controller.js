const gradesBulkService = require("./grades.bulk.service");
...
/**
 * رفع ملف Excel لإضافة الصفوف
 */
const bulkUploadGrades = async (req, res, next) => {
  try {
    if (!req.file) {
    ...
    // الأعمدة المطلوبة
    const requiredColumns = ["name", "monthly_price"];

    // التحقق من الأعمدة
    validateColumns(data, requiredColumns);

    // معالجة البيانات
    const result = await gradesBulkService.processGradesBulk(data);
    ...
    await logActivity({
      ...
      action: "bulk_upload_grades",
      entity_type: "grade",
      entity_id: null,
      description: `رفع ${result.success_count} صف من ملف Excel`,
    });
 ...
 */
const downloadGradesTemplate = async (req, res, next) => {
  try {
    const { downloadTemplate } = require("../../utils/excelTemplates");
    return downloadTemplate(res, "grades");
  } catch (error) {
    next(error);
  }
};
