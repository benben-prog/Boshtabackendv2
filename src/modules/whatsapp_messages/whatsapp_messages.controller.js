...
const sendWelcome = async (req, res, next) => {
  try {
    ...
    return res.status(201).json({
      success: true,
      message: "تم إضافة رسالة الترحيب للطابور",
      data: results,
    });
  } catch (error) {
    next(error);
...
const sendAbsence = async (req, res, next) => {
  try {
    ...
    const studentData = student.rows[0];
    const absenceDate = date || getTodayEgypt();
    ...
    return res.status(201).json({
      success: true,
      message: "تم إضافة رسالة الغياب للطابور",
      data: results,
    });
  } catch (error) {
    next(error);
...
const sendPayment = async (req, res, next) => {
  try {
    ...
    const paymentInfo = {
      month: paymentData.month || "غير محدد",
    ...
    return res.status(201).json({
      success: true,
      message: "تم إضافة رسالة الدفع للطابور",
      data: results,
    });
  } catch (error) {
    next(error);
...
const sendExam = async (req, res, next) => {
  try {
    ...
    if (!examResult.rows[0]) {
      throw new Error("نتيجة الامتحان غير موجودة");
    }
    ...
    return res.status(201).json({
      success: true,
      message: "تم إضافة رسالة النتيجة للطابور",
      data: results,
    });
  } catch (error) {
    next(error);
...
// ============================================
// QUEUE: Force process (manual trigger)
// ============================================

const forceProcess = async (req, res, next) => {
  try {
    const result = await whatsappDispatcher.forceProcess();
    ...
    await logActivity({
      ...
      entity_type: "whatsapp",
      entity_id: null,
      description: `Force process triggered - queue size: ${result.queue_size}`,
    });

    return res.status(200).json({
      success: true,
      message: "تم تشغيل معالجة الطابور",
      data: result,
    });
  } catch (error) {
    next(error);
...
const resetFailed = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: `تم إعادة تعيين ${result.length} رسالة`,
      data: result,
    });
  } catch (error) {
    next(error);
...
const toggleTemplateActive = async (req, res, next) => {
  try {
    ...
    await logActivity({
      ...
      entity_type: "whatsapp_template",
      entity_id: templateId,
      description: `Toggled WhatsApp template (ID: ${templateId}) to ${isActive ? "active" : "inactive"}`,
    });
    ...
    return res.status(200).json({
      success: true,
      message: isActive ? "تم تنشيط القالب" : "تم إلغاء تنشيط القالب",
      data: formattedTemplate,
    });
  } catch (error) {
    next(error);
